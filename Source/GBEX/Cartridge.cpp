...
namespace gbex
{
	...
	*/
	...
	uint16_t MBC5::read16(uint16_t addr)
	{
		if (addr >= 0x0000 && addr < 0x7FFF)
		{
			uint8_t upper = read8(addr);
		...
		throw std::runtime_error("Unimplemend MBC5 Logic or its not allowed: read16");
	}

	void MBC5::write8(uint16_t addr, uint8_t data)
	{
		// Enable RAM
		if (addr >= 0x000 && addr <= 0x1FFF)
		{
	...
	void MBC5::write16(uint16_t addr, uint16_t data)
	{
		uint8_t upper = data & 0xFF;
		uint8_t lower = (data >> 8) & 0xFF;
